...
<head>
    ...
    <script type="text/javascript" src="https://assets.mediadelivery.net/playerjs/pjs-implementation.js"></script>
    ...
    <meta property="og:image"
        content="https://{{ config('services.bunnycdn.pull_zone') }}.b-cdn.net/{{ $video->guid }}/thumbnail.jpg">
    ...
    <meta property="og:video:url"
        content="https://{{ config('services.bunnycdn.pull_zone') }}.b-cdn.net/{{ $video->guid }}/play_720p.mp4">
    ...
    <meta property="video:duration" content="00:21:29.0000000">
    ...
    <style>
        * {
        ...
        :root {
            --plyr-color-main: {{ env('PRIMARY_COLOR') }};
        ...
        [data-plyr="captions"].plyr__control--pressed {
            border-bottom: solid 3px {{ env('PRIMARY_COLOR') }};
        }
...
<body id="body">
        ...
        <svg viewBox="0 0 0 0" style="height: 0; width: 0; display: block;">
            <defs>
                ...
                <linearGradient id="gradient-active" x2="0%" y2="100%">
                    <stop offset="0%" stop-color="{{ env('PRIMARY_COLOR') }}af"></stop>
                    <stop offset="30%" stop-color="{{ env('PRIMARY_COLOR') }}8f"></stop>
                    <stop offset="70%" stop-color="{{ env('PRIMARY_COLOR') }}3f"></stop>
                    <stop offset="100%" stop-color="{{ env('PRIMARY_COLOR') }}13"></stop>
                </linearGradient>
            </defs>
        </svg>
        <video id="main-video" preload="auto" autoplay
            data-plyr-config="{ &quot;title&quot;: &quot;{{ $video->title }}&quot; }"
            data-poster="https://{{ config('services.bunnycdn.pull_zone') }}.b-cdn.net/{{ $video->guid }}/thumbnail.jpg">
            <source type="application/x-mpegURL"
                src="https://{{ config('services.bunnycdn.pull_zone') }}.b-cdn.net/{{ $video->guid }}/playlist.m3u8">
        </video>
    </div>

    @php
        $thumbnails = [];
        for ($i = 0; $i < $video->thumbnailCount / 36; $i++) {
            $thumbnails[] =
                'https://' .
                config('services.bunnycdn.pull_zone') .
                '.b-cdn.net/' .
                $video->guid .
                '/seek/_' .
                $i .
    ...
    <script>
        ...
        var seek_thumbnail_config = {
            enabled: true,
            pic_num: {{ $video->thumbnailCount }}, // total thumbnail numbers
        ...
        // var videoSessionTracker = new BunnyStreamSessionTracker("mediadelivery.net", "video-803.mediadelivery.net", {{ $video->videoLibraryId }},
        //     "{{ $video->guid }}", "803");
        var errorEvents = {};
        ...
        document.addEventListener("DOMContentLoaded", async function() {
            ...
            var originalUrl =
                "https://{{ config('services.bunnycdn.pull_zone') }}.b-cdn.net/{{ $video->guid }}/original";
            var urlPlaylistUrl =
                "https://{{ config('services.bunnycdn.pull_zone') }}.b-cdn.net/{{ $video->guid }}/playlist.m3u8";
                    ...
                    $('.chromecast').click(function() {
                        if (cjs.available) {
                            if (cjs.connected) {
                                ...
                                cjs.cast(castUrl, {
                                    poster: "https://{{ config('services.bunnycdn.pull_zone') }}.b-cdn.net/{{ $video->guid }}/thumbnail.jpg",
                                    title: 'Le g&#233;nie g&#233;n&#233;tique ',
                                    subtitles: []
                                })
                            }
                        }
            ...
            var defaultOptions = {
                storage: {
                    enabled: true,
                    key: "plyr--lib-{{ $video->videoLibraryId }}"
                },
                fullscreen: {
                    enabled: true,
                    fallback: true,
            ...
            function initPlayer() {
                ...
                function wireUpEvents() {
                    const events = ["progress", "ready", "play", "pause", "timeupdate", "seeking", "seeked",
                        "ended", "ratechange", "enterfullscreen", "exitfullscreen", "captionsenabled",
                        "captionsdisabled", "languagechange", "volumechange", "error", "fullscreenchange",
                    ]
