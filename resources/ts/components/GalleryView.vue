...
<template>
    <VCard class="position-sticky mb-5 pa-2" :style="{ top: searchPosisions || '0px', 'z-index': 10 }">
        <VRow>
            ...
            <VCol v-if="$slots.actions">
                <slot name="actions" />
            </VCol>
        </VRow>
    </VCard>
    <Masonry v-slot="{ item }" :grid="grid" :is-loading="isLoading" :items="media" @load="loadMore">
        <Media v-if="variant === 'preview'" :deletable="deletable" :media="item" is-preview @update:media="init" @on-delete="removeItem" />
            ...
            >
                <VImg :src="item.thumb_url" class="w-100 h-100 media-card" cover>
                    <VToolbar absolute density="compact" class="toolbar-actions">
                        <VCardSubtitle style="inline-size: 95%" class="text-white" :title="item.name">
                            {{ item.name }}
                        </VCardSubtitle>
                    </VToolbar>
                </VImg>
