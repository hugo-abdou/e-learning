...
const style = computed(() => ({
    aspectRatio: props.aspectRatio || "1.77",
}));
...
<template>
    ...
    >
        ...
        <div class="card-header-actions d-flex align-center justify-center w-100 pa-1">
            <VSpacer />
            <IconBtn
                v-if="resource.status === MediaStatus.Error || resource.status === MediaStatus.Processing"
                @click="retry"
                icon="tabler-reload"
            />
            <IconBtn
        ...
        <VToolbar
            absolute
            density="compact"
            class="toolbar-overlay pa-2"
        >
            <div class="d-flex align-center justify-center w-100 me-2">
                <IconBtn
                    v-if="resource.url && $can('read', 'storage')"
