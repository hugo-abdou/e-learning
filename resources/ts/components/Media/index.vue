...
import useSnackBarStore from "../SnackBar/useSnackBarStore";
...
const thumb = computed(
  () => resource.value.thumb_url || resolveDefaultThumbnal(resource.value.type)
);
  ...
  return [
    {
      ...
      props: {
        ...
        onClick: () => deleteMedia(resource.value),
      },
    },
    {
      title: "retry",
      _if:
        resource.value.status === MediaStatus.Pending ||
        resource.value.status === MediaStatus.Processing,
      props: { "prepend-icon": "tabler-refresh", onClick: refreshMedia },
    },
    {
      title: "retry",
      _if: resource.value.status === MediaStatus.Error || true,
...
<template>
  <VCard
    ...
    :style="style"
    color="surface"
    class="media-card w-100 h-100 overflow-hidden"
  >
