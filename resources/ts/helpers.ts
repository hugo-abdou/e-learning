...
export async function createFileFromDataString(
  dataString: string,
  fileName: string
) {
...
export function getAction(input: string): string {
  const matches = input.match(/^(.+)_/);
  return matches && matches[1] ? matches[1] : "";
}
...
export function calculateWordsToDisplay(
  containerWidth: number,
  text: string,
  opt: { charWidth?: number; maxLines?: number }
) {
  const charWidth = opt?.charWidth || 8;
  const maxLines = opt?.maxLines || 1;
  const estimatedCharCount = Math.floor(
    containerWidth / (charWidth * maxLines)
  );
