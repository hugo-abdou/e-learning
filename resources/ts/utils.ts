...
export const resolveMediaStatusVariant = (stat: number) => {
  if (stat === MediaStatus.Pending)
    return { lable: 'Pending', color: 'default', icon: 'basil:lock-time-outline' }
  if (stat === MediaStatus.Processing)
    return { lable: 'Processing', color: 'info', icon: 'healthicons:i-schedule-school-date-time' }
  if (stat === MediaStatus.Completed)
    return { lable: 'Completed', color: 'success', icon: 'material-symbols:published-with-changes-rounded' }
  if (stat === MediaStatus.Error)
    return { lable: 'Error', color: 'error', icon: 'codicon:error' }

  return { lable: 'Inconnue', color: 'default', icon: 'clarity:unknown-status-line' }
}
export const resolveCourseStatusVariant = (stat: string) => {
  const statLowerCase = stat.toLowerCase()
  if (statLowerCase === CourseStatus.Draft)
    return { color: CourseStatusColors.Draft, icon: 'mdi-database-outline' }
  if (statLowerCase === CourseStatus.Private)
    return { color: CourseStatusColors.Private, icon: 'mdi-eye-off-outline' }
  if (statLowerCase === CourseStatus.Scheduled)
    return { color: CourseStatusColors.Scheduled, icon: 'mdi-calendar-arrow-right' }
  if (statLowerCase === CourseStatus.Published)
    return { color: CourseStatusColors.Published, icon: 'mdi-earth' }
  if (statLowerCase === CourseStatus.Error)
    return { color: CourseStatusColors.Error, icon: 'mdi-alert-circle-outline' }
  if (statLowerCase === CourseStatus.Closed)
    return { color: CourseStatusColors.Closed, icon: 'mdi-eye-lock-outline' }
...
export const resolveCourseVisibilityVariant = (stat: number) => {
  if (stat === CourseVisibility.Visibil)
    return { color: 'success', icon: 'fluent:eye-12-regular' }
  if (stat === CourseVisibility.Hidden)
    return { color: 'default', icon: 'fluent:eye-hide-20-regular' }
