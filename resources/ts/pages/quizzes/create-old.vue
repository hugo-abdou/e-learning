...
import initialEditorData from "./components/initialEditorData.json";
definePage({
  meta: {
...
const onEditorChange = async (questionId: any, editor: EditorJS) => {
  editor.save().then((e: OutputData) => {
    const questionIndex = quiz.value.questions.findIndex(
      ({ id }) => id === questionId
    );
    quiz.value.questions[questionIndex].question = e;
  });
};
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 0; i < editorJs.value.length; i++) {
        ...
        quiz.value.questions[questionIndex].question = data;
      }
      ...
      const res = await quizStore.createQuiz({
        ...quiz.value,
        // @ts-ignore
        questions: quiz.value.questions.map(({ id, question, ...all }) => ({
          ...all,
    ...
    <VRow>
      ...
      <VCol cols="12" v-for="(question, index) in quiz.questions" :key="index">
        <VCard
          ...
          :title="`${'Question ' + (index + 1)} - (${question.options.reduce(
            ...
            <VRow>
              <VCol cols="12">
                <EditorJs
                  ...
                  :holder="question.id"
                />
                <!-- @editor-change="onEditorChange(question.id, $event)" -->
              </VCol>
              <template
