...
const submitNewChaptersOrder = debounce(() => {
    courseStore.reOrderChapters(chapters.value.map(({ id, order }) => ({ id, order })));
}, 1000);
...
const quizCreated = (quiz: Quiz) => {
    if (course.value) {
        courseStore.attachQuizzes(course.value.id, [...quizzes.value.map(({ id }) => id), quiz.id]);
