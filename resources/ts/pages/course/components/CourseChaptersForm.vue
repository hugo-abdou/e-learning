...
onMounted(() => {
    if (chaptersForm.value.length === 0) chaptersForm.value.push({ title: "", attachments: [], order: 0, id: 0 });
});
...
<template>
    <VForm ref="formEl">
        <VExpansionPanels v-model="currentChapter" :class="chaptersForm.length > 1 && 'no-icon-rotate'" variant="default">
            <VExpansionPanel v-for="(chapter, i) in chaptersForm" :key="i">
                <VExpansionPanelTitle>
                    <VCardTitle style="inline-size: 90%" class="text-capitalize d-flex pr-0">
                    ...
                    <template v-if="chaptersForm.length > 1" #actions>
                        <VMenu transition="speed-dial-left" location="left center">
                            ...
                            <VList class="mr-1 px-2 d-flex gap-1 bg-background">
                                <ActionButton variant="text" :disabled="i === 0" icon="mdi-arrow-up" @click.stop="up(i)" />
                                <ActionButton
                                    variant="text"
                                    ...
                                    @click.stop="down(i)"
                                />
                                <ActionButton variant="text" icon="tabler-trash" color="error" @click.stop="deleteChapter(i)" />
                            </VList>
                        </VMenu>
                    </template>
                </VExpansionPanelTitle>
                <VExpansionPanelText class="border-t">
                    <VCardText class="mb-5">
                        <VRow>
                            ...
                            >
                                <VRow :key="gridType" class="rounded-lg bg-background border w-100 ma-0">
                                    <VCol v-bind="gridType === 'list' ? { cols: '12', md: '4' } : { cols: '12' }">
                                        <Media
                                            :media="doc"
                                            deletable
                                            is-preview
                                    ...
                                    <VCol v-bind="gridType === 'list' ? { cols: '12', md: '8' } : { cols: '12' }">
                                        <VRow>
                                            <VCol cols="12">
                                                <VTextField
                                                    v-model="doc.name"
                            ...
                            <VCol cols="12">
                                <InfoTooltip v-slot="{ props }" :text="$t('media.upload.description')">
                                    <VBtn
    ...
    <div class="text-center">
        <VBtn class="mt-5" variant="tonal" append-icon="tabler-plus" rounded="pill" @click="addChapter">
            {{ $t("Add Chapter") }}
        </VBtn>
    </div>
    <FileUploaderDialog
