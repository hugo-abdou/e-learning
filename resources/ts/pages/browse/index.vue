<template>
    <div>
        ...
        <VRow>
            <VCol v-for="(course, index) in courses" :key="index" cols="12" sm="6" md="4" lg="3">
                <VCard>
                    <VImg :src="course.thumbnail || ''" cover height="200px" />
                    <VCardItem>
                        <VCardTitle>{{ course.title }}</VCardTitle>
                        <VCardSubtitle>
                            <VChip color="secondary">filming</VChip>
                        </VCardSubtitle>
                    </VCardItem>
                    <VCardText class="d-flex align-center gap-2">
                        <VAvatar color="primary" variant="tonal" icon="bi:book" />
                        <p v-if="course.chaptersCount" class="ma-0">{{ course.chaptersCount }} chapters</p>
                        <p v-else class="ma-0">no chapters</p>
                    </VCardText>
                    <VCardText>
                        <VProgressLinear rounded rounded-bar :model-value="50" :max="100" height="10" color="primary" />
