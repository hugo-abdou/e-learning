...
<template>
    <VCard title="Storage" :subtitle="`Total ${total_storage} Usage Rate`" class="h-100" :loading="isLoading">
        <template #append>
            <IconBtn @click="fetch()">
        ...
        <VCardText>
            <VRow align="center">
                <VCol cols="12" sm="6" order="2" order-sm="1" v-if="states.length">
                    <VList class="card-list d-flex flex-wrap">
                        <VCol cols="6" class="text-no-wrap pb-2" v-for="state in states" :key="state.name">
                            <VListItem>
                                <VListItemTitle class="font-weight-medium text-capitalize"> {{ state.name }} </VListItemTitle>
                                <VListItemSubtitle class="text-disabled"> count: {{ state.count }} </VListItemSubtitle>
                                <template #prepend>
                ...
                <VCol style="margin-top: -20px" order="1" order-sm="2" class="d-flex justify-center align-center pa-0 pb-5">
