...
import type { UserWithChat, User } from "@/types";

interface Props {
  isChatContact?: boolean;
  user: User | UserWithChat;
}
  ...
  >
    ...
    <div class="flex-grow-1 ms-4 overflow-hidden">
      ...
      <p class="mb-0 text-truncate text-disabled">
        {{ props.isChatContact && "chat" in props.user ? props.user.chat.lastMessage?.message : "" }}
      </p>
    </div>
    <div
...
.chat-contact {
  ...
  &.chat-contact-active {
    background: linear-gradient(72.47deg, rgba(var(--v-theme-primary), 0.7) 0%, #fff 300%);
    color: #fff;
