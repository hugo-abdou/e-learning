...
const resolveFeedbackIcon = (feedback: ChatOut["messages"][number]["feedback"]) => {
  if (feedback.isSeen) return { icon: "tabler-checks", color: "info" };
  ...
  <div class="chat-log pa-5">
    ...
    >
      ...
      >
        <!-- @vue-skip -->
        <Avatar
          :user="msgGrp.senderId === contact.id ? contact : store.profileUser"
          :size="32"
        />
      </div>
      <div
      ...
      >
        <p
          ...
          :style="{
            backgroundColor: `rgba(var(${msgGrp.senderId === contact.id ? '--v-theme-surface' : '--v-theme-primary'}),0.7)`,
          }"
          :class="[msgGrp.senderId === contact.id ? 'chat-left' : 'text-white chat-right', msgGrp.messages.length - 1 !== msgIndex ? 'mb-3' : 'mb-1']"
        >
          {{ msgData.message }}
        </p>
