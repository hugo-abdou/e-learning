...
const resolveInvoiceStatusVariantAndIcon = (status: string) => {
    if (status === "Partial Payment") return { variant: "warning", icon: "tabler-circle-half-2" };
    if (status === "Paid") return { variant: "success", icon: "tabler-circle-check" };
    if (status === "Downloaded") return { variant: "info", icon: "tabler-download" };
    if (status === "Draft") return { variant: "secondary", icon: "tabler-device-floppy" };
        ...
        <VTable class="text-no-wrap invoice-list-table">
            ...
            <tbody>
                ...
                >
                    <!-- 👉 Id -->
                    <td>
                        <RouterLink :to="{ name: 'apps-invoice-preview-id', params: { id: invoice.id } }"> #{{ invoice.id }} </RouterLink>
                    </td>
