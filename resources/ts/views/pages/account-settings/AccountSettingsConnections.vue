    ...
    <VRow>
        ...
        <VCol cols="12" md="6">
            <VCard title="Connected Accounts">
                <VCardText>
                    <p class="mt-n4 mb-6 text-sm">Display content from your connected accounts on your site</p>
                    <VList class="card-list">
                        <VListItem v-for="item in connectedAccounts" :key="item.logo" :title="item.name">
                            <template #prepend>
                                <VAvatar start>
        ...
        <VCol cols="12" md="6">
            <VCard title="Social Accounts">
                <VCardText>
                    <p class="mt-n4 mb-6 text-sm">Display content from social accounts on your site</p>
                    <VList class="card-list">
                        <VListItem v-for="item in socialAccounts" :key="item.logo" :title="item.name">
                            <template #prepend>
                                <VAvatar start>
                                    <VImg :src="item.logo" height="30" />
                                </VAvatar>
                            ...
                            <VListItemSubtitle v-if="item.links?.link" tag="a" :href="item.links?.link" style="opacity: 1">
                                {{ item.links?.username }}
                            </VListItemSubtitle>
                            ...
                            <template #append>
                                <VListItemAction>
                                    <VBtn
                                        ...
                                        class="rounded"
                                    >
                                        <VIcon size="20" :icon="item.connected ? 'tabler-trash' : 'tabler-link'" />
                                    </VBtn>
                                </VListItemAction>
                            </template>
                        </VListItem>
