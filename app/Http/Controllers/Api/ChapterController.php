...
class ChapterController extends Controller
{
    ...
    public function store(ChapterRequest $request)
    {
        ...
        try {
            ...
            $courseDuration = $course->chapters->reduce(function ($prev, $ch) {
                return $prev + $ch->attachments->sum('data.duration');
            }, 0);
    ...
    public function student_chapters(Request $request)
    {
        dd('student_chapters');
        $chapters = Course::where('slug', $request->get('course_id'))->firstOrFail()
            ->chapters()->orderBy('order')->with(['attachment_resources', 'attachment_resources.progress', 'attachment_resources.media'])->get();
