...
class CryptoJSDES
{
    ...
    {
        $salt = $salt ?: openssl_random_pseudo_bytes(16);
        list($key, $iv) = self::evpkdf($passphrase, $salt);

        $ct = openssl_encrypt($data, 'AES-128-CBC', env('VITE_SECRET_KEY'), true, 'gehydkishpejhypl');
    ...
    public static function decrypt($base64, $passphrase)
    {
        ...
        $data = openssl_decrypt($ct, 'AES-128-CBC', $key, true, $iv);
